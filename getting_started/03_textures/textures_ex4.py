...
def main():
    ...
    # shaders
    shader = Shader('vertex.glsl', 'fragment_ex4.glsl')
...
def process_input(window):
    ...
    if get_key(window, KEY_UP) == PRESS:
        mix_value = min(mix_value + 1e-3, 1.0)

    if get_key(window, KEY_DOWN) == PRESS:
        mix_value = max(mix_value - 1e-3, 0)
